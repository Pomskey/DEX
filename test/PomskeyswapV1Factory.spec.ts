...
describe('PomskeyswapV1Factory', () => {
  let wallet: Wallet, other: Wallet

  let factory: PomskeyswapV1Factory
  let poolBytecode: string
  const fixture = async () => {
    const factoryFactory = await ethers.getContractFactory('PomskeyswapV1Factory')
    return (await factoryFactory.deploy()) as PomskeyswapV1Factory
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('PomskeyswapV1Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('PomskeyswapV1Pool')
