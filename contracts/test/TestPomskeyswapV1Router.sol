...
contract TestPomskeyswapV1Router is IPomskeyswapV1SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IPomskeyswapV1Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function swapForExact1Multi(
        ...
        address[] memory pools = new address[](1);
        pools[0] = poolInput;
        IPomkseyswapV1Pool(poolOutput).swap(
            recipient,
            true,
            -amount1Out.toInt256(),
    ...
    function PomskeyswapV1SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IPomskeyswapV1Pool(msg.sender).token0() : IPomkseyswapV1Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IPomkseyswapV1Pool(pools[0]).token1();
            IPomskeyswapV1Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IPomskeyswapV1Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IPomkseyswapV1Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
